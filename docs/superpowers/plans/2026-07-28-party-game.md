...
- [ ] parseHash 테스트 작성(`#/idiom?mode=solo` → `{page:'idiom', params:{mode:'solo'}}`, 빈 해시 → home) → 실패 확인 → 구현 → 통과
- [ ] index.html: `<main id="app">` + 홈 그리드(7게임 카드, 이모지+제목), 게임 화면은 mount 방식
- [ ] style.css: 다크 배경(#111 계열) + 노랑 포인트(예능 자막 느낌), 카드 그리드, 큰 버튼(모바일 터치)
- [ ] sound.js: OscillatorNode 합성 5종, 첫 터치에서 AudioContext resume
- [ ] 브라우저 확인(홈 → 카드 클릭 → 빈 게임 화면 → 뒤로), 커밋
...
- [ ] choseong 테스트(받침 있는 글자, 혼합 문자열, 빈 문자열) → 구현(유니코드 0xAC00 분해) → 통과
- [ ] validate-data.mjs: 파일별 검사 함수(idioms/celebs/charades/liar/choseong-dict), 존재하는 파일만 검사
- [ ] 커밋
...
- [ ] 난이도별로 직접 작성: level1 150개(고사성어 교과서 수준), level2 150개, level3 100개. 뜻풀이는 한 문장 요약으로 직접 작성
- [ ] validate-data.mjs에 idioms 검사(4글자, front+back=word, 중복 없음, meaning 10자+) → 통과 확인
- [ ] 커밋
...
- [ ] 카테고리별 직접 작성(가수 100, 배우 100, 예능인 60, 운동선수 40). 힌트는 사실 정보만, 3단계 점점 구체적으로. wiki는 한국어 위키 문서 제목
- [ ] validate 검사(cho가 choseong(name)과 일치, 힌트 3개, 중복 없음) → 통과
- [ ] 커밋
...
- [ ] 카테고리별 작성 → validate(카테고리당 100+, 중복 없음) → 커밋
...
- [ ] 카테고리(장소/음식/직업/물건/동물) 100세트 직접 작성 → validate → 커밋

### Task 7: 초성 사전 생성

**Files:** Create `tools/build-choseong-dict.mjs`, `web/data/choseong-dict.json`, `tests/build-dict.test.mjs`

- [ ] WordChain `app/src/main/assets/dict_common.txt` 포맷 확인(줄 단위 단어 목록 가정, 다르면 파서 조정)
- [ ] 변환: 2글자 완성형 한글 단어만 → 초성쌍("ㄱㅅ")별 그룹 → 30단어 미만 쌍 제외 → JSON `{"ㄱㅅ":["가수","감사",...]}`
- [ ] 그룹핑 함수 테스트(소형 입력) → 스크립트 실행 → validate → 커밋
...
- [ ] 퀴즈 상태머신 테스트(정답/오답/힌트 감점/라운드 진행/최종 집계) → 구현 → 통과
- [ ] rival 테스트(1000회 샘플링 평균이 파라미터 ±30%, level별 단조성) → 구현 → 통과
- [ ] 커밋
...
- [ ] makeChoices 테스트(정답 포함, 4개 유일, pool 부족 시 에러 없이 가능한 만큼) → 구현
- [ ] 화면: 모드 선택(혼자 연습/AI 대결/다같이) → 난이도 → 라운드 10개. AI 대결은 타이머 바에 AI 예상 응답 시점 표시, 먼저 맞추면 승. 오답 시 뜻 힌트 후 1회 재도전
- [ ] 브라우저 수동 확인 → 기록 저장(최고점) → 커밋
...
- [ ] 초성 모드: celebs.json + quiz 엔진, 힌트 3단계 공개. AI 대결 지원
- [ ] 사진 모드: 라운드 시작 시 fetchPortrait, 모자이크 level 8→1 시간 경과 공개(또는 실루엣), 정답 공개 시 원본+출처 링크 표시. null이면 그 라운드는 초성 모드로 자동 전환. navigator.onLine false면 모드 선택에서 사진 모드 비활성 표시
- [ ] 힌트 스케줄 순수 함수 테스트 → 브라우저 확인(온라인/오프라인 시뮬) → 커밋
...
- [ ] classifyTilt 테스트(경계값·히스테리시스) → 구현
- [ ] 화면: 카테고리·시간 선택 → "이마에 대세요" 카운트다운 → 전체화면 큰 글자 제시어, 기울임 판정+효과음, DeviceOrientation 권한 없거나 미지원이면 화면 상/하 터치 버튼. Screen Wake Lock(지원 시)
- [ ] 결과 화면(맞춤/패스 목록) → 실기기 확인 → 커밋
...
- [ ] checkWord·pickAiWord 테스트 → 구현
- [ ] 화면: 파티(폰 돌리며 입력, 숨은 랜덤 타이머 15~45초, 폭발 연출+사운드) / 솔로(AI와 교대, AI 응답 딜레이 연출). 초성쌍은 dict에서 랜덤
- [ ] 브라우저 확인 → 커밋
...
- [ ] 역할 배정·솔로 라운드 조립 테스트(라이어 정확히 1명, 셔플 후 인덱스 일치) → 구현
- [ ] 파티 플로우: 인원 설정 → "N번째 사람 확인" 넘겨보기(터치로 가림/공개) → 토론 안내 → 투표 집계 → 라이어 공개
- [ ] 솔로 플로우: AI 4명 말풍선 순차 공개 → 지목 → 정답 공개·연속 정답 기록
- [ ] 브라우저 확인 → 커밋
...
- [ ] winnerAt 테스트(경계각) → 구현
- [ ] canvas 룰렛: 항목 입력(이름/벌칙, 프리셋 벌칙 15종 내장), ease-out 회전 4~6초, 당첨 연출+fanfare
- [ ] 브라우저 확인 → 커밋
...
- [ ] 문제 만들기: `<input type=file accept=image/*>` 여러 장 → 각 사진에 정답 이름 입력 → 메모리(세션)에만 보관, 안내 문구 "사진은 기기 밖으로 나가지 않아요"
- [ ] 출제: photofx.mosaic 재사용, 점점 선명 → 정답 공개. 진행자가 정답 판정(맞음/넘김 버튼)
- [ ] 브라우저 확인 → 커밋
...
- [ ] 홈에 정보(ⓘ) 페이지: 데이터 출처(직접 제작, 사전 CC0, 사진은 위키피디아 실시간·미저장), 오픈소스 고지, privacy.html 링크
- [ ] privacy.html: 수집 정보 없음, localStorage 로컬 저장만, 사진 모드 시 위키피디아 접속 고지
- [ ] 커밋
...
- [ ] workflow: push(main) → `actions/upload-pages-artifact`(path: web) → `actions/deploy-pages`
- [ ] push → Actions 성공 확인 → `https://yongminlee2.github.io/partyGame/` 동작 확인(fetch로 200 확인)
- [ ] 커밋
...
- [ ] 구조: settings.gradle.kts + app 모듈, applicationId `com.ymgames.partygame`(가칭), minSdk 24 / targetSdk는 WordChain과 동일
- [ ] MainActivity: WebView + `WebViewAssetLoader`(`https://appassets.androidplatform.net/assets/web/index.html`) — ES 모듈·fetch 동작, JS·DOM storage 활성, 뒤로가기=라우터 back, 화면 회전 대응
- [ ] gradle 태스크 `copyWebAssets`: `web/` → `app/src/main/assets/web/` (assets는 .gitignore)
- [ ] `gradlew assembleDebug` 성공 → adb로 S20 Ultra 설치·전 게임 스모크 테스트(사진 모드는 온라인) 
- [ ] 커밋
...
- [ ] 구성: 소개·게임 목록(스크린샷 없이 표)·아키텍처 요약·저작권 분석 정리(사진 3안 검토 결과 포함)·개발일지(태스크별 날짜·내용·부딪힌 문제)·빌드/배포 방법·플레이스토어 준비 상태
- [ ] 이후 태스크 진행마다 개발일지 섹션 갱신 (완료 시점 일괄 아님)
- [ ] 커밋·푸시

### Task 20: 최종 검증

- [ ] `node --test tests/` 전체 통과 + `node tools/validate-data.mjs` 통과
- [ ] 웹(Pages URL)에서 7게임 전부 실행 확인, APK 오프라인 실행 확인
- [ ] README 개발일지 마감 갱신 → 커밋 → push
